...
A designated observation requires the user or orchestrator to designate it before execution, or task context or a packet supplied by another authority or independently approved before execution to identify the opportunity. The executing agent cannot self-designate through its own packet or context; an opportunity noticed after execution begins can only be proposed for a future task. A comparable known-version observation also requires the task's source/version from the designating authority or a contemporaneous acquisition tied to the guidance read during the task. Without it, retain the facts only as a primary-trigger supporting case or an explicitly version-inconclusive observation, never as known-version effectiveness evidence.

For `coding-review-loop`, the Task-Start Skill Identity step makes this information available without asking the user to inspect commits. A read-only helper inside the installed package computes its content fingerprint at first use; the agent retains it in task context, reads guidance from that directory, and checks again at case capture. Every new case records start/end fingerprints and acquisition timing, or an explicit failure/uncertainty reason. The receipt may travel in an already-required packet or handoff, but creates no standalone artifact, invocation log, or additional case-capture trigger. Changed packages, late acquisition, missing start receipts, or uncertain read timing prevent an unqualified single-version attribution. This is observed package identity tied to reading, not a platform attestation of model loading or rule execution. Historical cases remain unknown unless contemporaneous evidence exists. Other skills retain their existing capture behavior.
...
The target-project user records only local facts. For new `coding-review-loop` cases, the executing agent supplies the Task-Start Skill Identity receipt or explicit attribution limit; the user need not reconstruct versions. Other source/version facts may be supplied by the designating authority or already visible during the task. Do not infer historical versions from current installations. The target project does not own inbox or casebook routing, skill refinement, evaluation decisions, lineage, root-cause classification, change/no-change decisions, promotion, regression classification, or later outcomes. The evolving-agent-dev distiller owns those central responsibilities.

A designated observation with independently established authority, a real opportunity, and the task's contemporaneously established or authority-supplied source/version can preserve a successful trigger or correct non-trigger fact for later comparison, but it cannot assign a central outcome. A version-inconclusive observation is supporting evidence only. Central distillation determines comparability and any later `improved`, `repeated`, or `inconclusive` classification from versioned evidence.
