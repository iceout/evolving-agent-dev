...
### 2026-06-19 - External mitigation versus policy redesign planning boundary

- Context: v0.3 external evidence only from a debugging and planning task in an external private code project after Stage 0 exited with frozen counts. The agent initially moved toward a conservative mitigation patch after identifying a scoring and selection issue, and the user had to redirect toward broader strategy redesign.
- What happened: The revised plan became more useful by separating score source, confidence, maturity, actionability, additive schema behavior, production-like verification, and reviewer-visible user decisions. The friction was that interim mitigation could be mistaken for final policy redesign and could anchor later plans too conservatively.
- Why it felt wrong: Future planning prompts should ask whether a proposed change is interim mitigation, final policy redesign, diagnostic-only analysis, or a staged plan that explicitly separates those phases. Future review packets should expose the boundary between temporary stopgap behavior and durable scoring or product policy.
- Category: `process` / `planning phase boundary`; secondary: `requirements` / `policy redesign boundary`; `review` / `review packet missing`; `context` / `user decision visibility`; `testing` / `production-like verification planning`
- Follow-up: This does not change Stage 0 counts and does not create E008, casebook entry, policy, ADR, automation, tooling, metrics, subagent framework, or docs/process-v0.3.md. It supports future v0.3 work on a minimal planning / review packet before subagent automation. Key lesson: review should distinguish interim mitigation, final policy redesign, diagnostic-only analysis, and staged plans, so durable constraints do not get replaced by longer natural-language plan text.

### 2026-06-21 - External reviewable constraints helped implementation review

- Context: v0.3 external evidence only from implementation after an accepted plan in an external private code project. The implementation mostly followed the plan and review feedback.
- What happened: Plan/review-packet style constraints helped the implementation trace dataflow and order, contract boundary, collection and counting behavior, safety boundaries, reuse decisions, failure and status behavior, time-window semantics, schema and status compatibility, and behavior-level tests.
- Why it felt useful: Reviewable constraints made implementation review easier than chat-only planning because the reviewer could compare code behavior against explicit boundaries instead of reconstructing the accepted plan from conversation memory.
- Remaining gap: Source-of-truth and fallback choices still need explicit packet fields. In this task, implementation used an existing signal as a coverage and maturity proxy, with fallback behavior, instead of adding a new runtime query path. Future packets should make that derived proxy and fallback decision reviewable.
...
### 2026-06-30 - Test-friendly IO abstraction polluted Mongo access

- Context: Reflection on agent-generated Mongo access code in an external project.
- What happened: Agent-generated code overused dependency-injection collection getters and thin Mongo collection wrapper functions. A temporary `audit -> audit2` fallback for `processed_log` also appeared to become tested behavior before the real project data-source facts were verified.
- Why it felt wrong: Tests became easier to patch, but production business paths accumulated ambiguous `get_collection` parameters, duplicated collection entrypoints, and infrastructure semantics that may contradict the source of truth. The issue was not that dependency injection or wrappers are always wrong; it was that test convenience and local compatibility shaped production APIs without a source-of-truth and reuse scan.
- Category: `testing` / `test seam pollution`; secondary: `implementation` / `thin wrapper proliferation`; `context` / `source-of-truth missing`; `process` / `reuse scan missing`
- Follow-up: Treat as reusable friction evidence only. Future IO helper, fallback, collection getter, adapter, or dependency-injection seam changes should first ask whether the behavior is real business strategy or test convenience, whether the data source is verified in docs/config/runtime evidence, whether tests can mock a single boundary helper instead of adding parameters through production APIs, and whether a wrapper adds semantics beyond `get_<mongo_config>_collection(name)`. This does not promote an active policy, create an evaluation, change Stage 0 counts, or expand v0.3 scope.

### 2026-06-30 - Review-fix patch duplicated fake Mongo cursor

- Context: Reflection on a real-project review-fix task after a subagent finding about raw Mongo cursor API compatibility.
- What happened: The agent fixed the immediate production path and failing tests, but added separate `_FakeMongoCursor` helpers in multiple test files instead of scanning for sibling fake cursor implementations and centralizing shared cursor semantics.
- Why it felt wrong: The fake looked like a local test helper, but it modeled Mongo cursor chaining and `sort()` / `limit()` behavior, including boundary behavior such as `limit(0)`, so small semantic drift could make tests unreliable. The review-fix step became too narrow: it patched the reported failure without re-running a focused reuse scan for newly introduced test doubles or adapter helpers.
- Category: `process` / `review-fix reuse scan missing`; secondary: `testing` / `duplicated test double`; `implementation` / `adapter helper duplication`; `review` / `narrow subagent finding fix`
- Follow-up: Treat as reusable friction evidence only. When fixing review or subagent findings, if the fix introduces or changes a helper, fake, adapter, wrapper, serializer, or test double, run a focused reuse scan across sibling tests/modules before finalizing. Test doubles that model external APIs with stateful, chained, or boundary semantics should be centralized or deliberately justified as local. This does not promote an active policy, create an evaluation, change Stage 0 counts, or expand v0.3 scope.

### 2026-07-01 - Runnable acceptance missing from review packet

- Context: Reflection on a real-project coding-review-loop task after subagent review and review-finding fixes.
- What happened: A new session still found that default trial-run scripts did not exercise the promised target behavior, shadow or distribution reports did not expose the promised 30d/90d cap-impact matrix, and an updated runscript parameter broke existing entrypoint tests that were not rerun.
