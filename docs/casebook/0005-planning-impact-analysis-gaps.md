...
A planning task can acknowledge uncertainty but still miss the impact of source-location accuracy, tool capability boundaries, runtime semantics, downstream callers, reusable shared logic, and behavior-level performance verification.

## Observed Behavior

Two external planning reviews showed a repeated planning-impact pattern:
