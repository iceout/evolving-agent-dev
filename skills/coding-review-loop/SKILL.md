...
## Task-Start Skill Identity

On first use in a task, before planning or implementation, run `python3 -B <loaded-skill-directory>/scripts/skill_identity.py` using the actual directory from which you read this skill, not an assumed installation directory or the target project's working directory. The helper ships with this package, uses only the standard library, and prints a `sha256-v1` identity covering the whole package, including references. It writes no files and needs no central repository or Git checkout.

Retain the fingerprint in task context, then read the skill and relevant references from that same directory. At case capture, rerun the helper and include the start/end fingerprints and acquisition timing in the existing note. Matching snapshots support attribution to the package read during this task; they do not audit model loading or rule execution. If the package changes during the task, retain both identities, identify affected phases when known, and mark single-version attribution inconclusive. Do not replace the start identity with the end identity. Hashing is not atomic; visible concurrent edits or uncertain reference loading also make attribution inconclusive.

If Python or the helper is unavailable, record `unknown` with the concrete reason when capturing a case; continue the authorized task without installing dependencies. If collection starts late or the start receipt is lost across a handoff/context reset, label it `late observation` or `start receipt unavailable`, never a task-start version. Preserve the receipt in an already-required packet/handoff when needed for continuity; do not create a usage log, extra artifact, or case merely to retain a fingerprint. Do not reconstruct old task versions from current disk state or Git history.
...
In that designated context, capture may record a correct trigger, nearby correct non-trigger, successful early constraint exposure, whether a reviewer avoided reconstructing implicit context, or non-recurrence where the task presented a real failure opportunity. A comparable known-version observation requires the task's skill source/version supplied by the designating authority or established by the Task-Start Skill Identity step. Do not investigate history to obtain it. If attribution is unavailable or inconclusive, record the facts only as an ordinary supporting case when a primary trigger applies, or as an explicitly version-inconclusive observation when the designating authority still requests capture; do not call either one a known-version comparable observation or effectiveness evidence. Acquiring a fingerprint does not designate an observation or establish effectiveness.

Ordinary successful tasks do not automatically become observations. Do not capture each invocation because it might be useful later, and do not treat a smooth task without a real comparable opportunity as effectiveness evidence. Target-project users do not need to provide lineage IDs, inspect skill commits, or classify an observation as improved, repeated, or regressed. Every new case includes the task's identity receipt or an explicit acquisition failure/attribution limit; do not silently omit it for ordinary cases.
